...
- `financing_simulations` — adds `credere_simulation_uuid`, `provider`, `raw_response`, `licensing_uf`, `licensing_city`, `seller_cpf`

## Security
- RLS enabled on all new tables, scoped to authenticated users via dealer ownership
- `credere_token_cache` is locked down: only service role can read/write; authenticated users get nothing
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'financing_simulations' AND column_name = 'consent_given') THEN
    ALTER TABLE financing_simulations ADD COLUMN consent_given boolean DEFAULT false;
  END IF;
END $$;
